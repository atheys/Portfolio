...
def vSev(dx,vx,t):
    vSev = (vZone-(dx+vx*t))/vZone
    return
def hSev(dy,dz,vy,vz,t):
    hSev = 1.-sqrt((dy+vy*t)*(dy+vy*t)+(dz+vz*t)*(dz+vz*t))/hZone
    return
def solver(dx,dy,dz,vx,vy,vz):
    a = fsolve(hSev(dy,dz,vy,vz,t)-vSev(dx,vx,t),0.)
    return a
...
def intrusion(datapoint1,datapoint2,ts):
    ...
    #print intrusion_vert,trouble_vert,t1_vert,t2_vert
    ...
    if (intrusion_vert):
        ...
        #print intrusion_hor,trouble_hor,t1_hor,t2_hor
    ...
    if (intrusion_hor) and (not(trouble_hor)) and (not(trouble_vert)):
        ...
        if (intrusion):
            ...
            D = coeff[1]**2-4*coeff[0]*coeff[2]
            if (D>=0.):
                root1 = (-coeff[1]+sqrt(D))/(2*coeff[0])#(-sqrt((2.*a*h*h*x-2*b*v*v*y-2*c*v*v*z)**2-4.*(a*a*h*h-b*b*v*v-c*c*v*v)*(h*h*x*x-v*v*y*y-v*v*z*z))-2*a*h*h*x+2*b*v*v*y+2*c*v*v*z)/(2.*(a*a*h*h-b*b*v*v-c*c*v*v))#-coeff[1]+sqrt(D))/(2*coeff[0])
                root2 = (-coeff[1]-sqrt(D))/(2*coeff[0])#(sqrt((2.*a*h*h*x-2*b*v*v*y-2*c*v*v*z)**2-4.*(a*a*h*h-b*b*v*v-c*c*v*v)*(h*h*x*x-v*v*y*y-v*v*z*z))-2*a*h*h*x+2*b*v*v*y+2*c*v*v*z)/(2.*(a*a*h*h-b*b*v*v-c*c*v*v))#(-coeff[1]-sqrt(D))/(2*coeff[0])
                roots = [root1,root2]
