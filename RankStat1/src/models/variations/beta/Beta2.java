 ...
 * @version 1.0
 */
public class Beta2 extends Model implements ModelFunctions {
	 ...
	 */
	public Beta2(String id, StatsBase sb, double[] wOffCorrelation, double[] wDefCorrelation)
			throws ModelException {
		super(id, sb);
		assert wOffCorrelation.length == 72 && wDefCorrelation.length == 72;
	 ...
	 * @throws ModelException
	 */
	private void developBiPoisson() throws ModelException {
		...
		assert home_values.length == away_values.length;
		...
		Poisson H = new Poisson(a_A * b_H * home_adv.getAvGoals());
		Poisson A = new Poisson(a_H * b_A * away_adv.getAvGoals());
		this.bp = new BiPoisson(this.id, A, H);
	}
