 ...
 * @version 2.0
 */
public class Beta3 extends Model implements ModelFunctions {
	 ...
	 */
	public Beta3(String id, StatsBase sb, double[] wOffCorrelation, double[] wDefCorrelation) {
		super(id, sb);
	...
	private void developBiPoisson() {
		...
		for (int i = 0; i < home_values.length; i++) {
			home_bonus[i] = findHome(i, home_values[i]);
			away_bonus[i] = findAway(i, away_values[i]);
		}
		...
		Poisson H = new Poisson(a_H * b_A * home.getAvGoals());
